...
exports.getMarriagesByYear = async (req, res) => {
  try {
    const { year, maleAgeGroup, femaleAgeGroup } = req.query;
    if (!year) {
    ...
    if (maleAgeGroup) {
      query += ` AND [MaleAgeGroup] = ?`;
      params.push(maleAgeGroup);
    }

    if (femaleAgeGroup) {
      query += ` AND [FemaleAgeGroup] = ?`;
      params.push(femaleAgeGroup);
    }
    ...
    if (!results || results.length === 0) {
      return res.status(404).json({
        error: {
          ...
          status: 404,
          filters: { year, maleAgeGroup, femaleAgeGroup },
        },
      });
    }
